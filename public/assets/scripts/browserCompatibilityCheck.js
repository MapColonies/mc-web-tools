...
var isBrowserVersionSupported = Number(userAgentRes.browser.version.split('.')[FIRST_ELEM]) >= MINIMUM_SUPPORTED_BROWSER_VERSION;
...
if (!isEngineChromium || !isBrowserVersionSupported) {
  ...
  window.location.replace(`${window._env_.PUBLIC_URL}/assets/pages/compatibilityError.html?error=${err}`);
}